...
SELECT CAST(d.week_stop_date AS DATE) AS week_, cs.BusinessUnit_, cs.Client_, CAST(cs.Employee as VARCHAR) as expert_id,  
    col_name As Smart_Offer_behavior,
    sum(col_value) as num,
    sum(Sales_Opportunity_Connected_Home) as den
FROM  Call_Stats cs
CROSS JOIN UNNEST(
    MAP(
        ARRAY[
            'Ask_for_the_Sale_c',
            'Non_Partner_Coverage_c',
            'Engagement_Questions_c',
            'Future_Purchase_Details_c',
            'Celebrate_Enrollment_c',
            'Ask_to_Send_Enrollment_Link_c',
            'Examples_of_Coverage_c',
            'Resolve_the_Customers_Concern_c',
            'RRR_c',
            'Price_c',
            'Discovery_Questions_c',
            'Transition_Statement_c'
        ],
        ARRAY[
            Ask_for_the_Sale_c,
            Non_Partner_Coverage_c,
...
group BY d.week_stop_date, col_name, BusinessUnit_, Client_, Employee
